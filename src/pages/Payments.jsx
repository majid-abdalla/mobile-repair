...
export default function Payments() {
  ...
  // ← Filter items marka hore (isla Repairs pattern-ka)
  const filteredItems = useMemo(() => {
    if (!methodFilter) return items
    return items.filter((item) => item.paymentMethod === methodFilter)
  }, [items, methodFilter])

  const { data, currentPage, totalPages, goToPage, resetPage } = usePagination(filteredItems, {
    pageSize: 10,
    searchFields: ['paymentId', 'repairId', 'paymentMethod', 'receivedByName'],
      ...
      <Card padding="none">
        <div className="space-y-4 p-4 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            ...
            <Select
              value={methodFilter}
              onChange={(e) => { setMethodFilter(e.target.value); resetPage() }} // ← e.target.value
